...
function Cards() {
  ...
  const redirectDatailCard = (id:string): void => {
    if (id.length > 0) {
  ...
  return (
    <section className="h-screen w-screen font-bol">
      <nav className="w-screen">
        <Header />
      </nav>
      <h1 className="text-center text-6xl font-semibold ">Catálogo de Cards</h1>
      <InputSearchCard />
      <div className="flex flex-wrap justify-center items-center">
        {currentData &&
          currentData.map((card: Card, index) => {
            return (
              <div
              ...
              >
                ...
                <img
                  ...
                  onClick={
                    (event: React.MouseEvent<HTMLImageElement, MouseEvent>)=> redirectDatailCard(event.currentTarget.id)
                  }
                  src={card.imageUrl}
                  alt="cards"
                />
